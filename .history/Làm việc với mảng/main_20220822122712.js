...
// Làm việc với MẢNG - Phần 2
    ...
    var isFreeSome = arrCoursesSome.some(function(course, index) {
        // Lặp qua từng phần tử của mảng
        // Mỗi lần lặp qua 1 phần tử sẽ gọi lại hàm đã truyền và truyền cho hàm đó tham số
        // Kiểm tra biểu thức điều kiện, nếu có 1 phần tử  thỏa mãn -> trả về true và dừng lặp
        return course.price === 0;
    }); //Call back
    ...
    //  8. reduce()
    // Hàm này chấp nhận 1 giá trị khởi điểm của output, sau đó thì sẽ lặp từng phần tử theo 1 công thức (tương tự như .map()), rồi cộng dồn giá trị vào output.
    ...
    function arrToObj(arr) {
        return arr.reduceCustom(function(info, key) {
            info[key[0]] = key[1]
            console.log('đây 1', info[key[0]] )
            console.log('đây 2', key[1] )
            console.log('đây 3',            info[key[0]] = key[1]
            )
            return info
        }, {})
    }
