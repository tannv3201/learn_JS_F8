    ...
    if (formElement) {
        ...
        for (var input of inputs) {
            ...
            for (var rule of rules) {
                ...
                // Nếu là rule cần value thì gán lại ruleFunc là ruleFunc được thực thi với giá trị truyền vào là ruleInfo[1]
                // ruleInfo[1]: là phần tử thứ 2 trong mảng rule được split [min, 6]
                if (isRuleHasValue) {
                    ruleFunc = ruleFunc(ruleInfo[0])
                }

                // Nếu formRules của input có name là một mảng 
                if (Array.isArray(formRules[input.name])) {
                    formRules[input.name].push(ruleFunc);
                } else {
                    // Đưa func vào mảng lần 1.
                    formRules[input.name] = [ruleFunc];
                    console.log(formRules[input.name] = [ruleFunc])
                }
            }
